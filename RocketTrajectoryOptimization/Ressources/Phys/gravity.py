...
class EarthGravity():
    ...
    def __call__(self, pos: np.ndarray) -> float:
        ...
        position = tools.cartesian_to_spheircal(pos)
        position = tools.orbitalspherical_conv(position)
        ...
        fg_p = (6*self.MU*(self.RE**2)*self.J22*math.sin(2*(position[1]-self.L22))*(math.cos(position[2])**2))/((position[0]**4)*math.sin((90*math.pi/180)-position[2]))

        accels = np.array([fg_r, fg_t, fg_p])
        accels = tools.accel_spherical_to_cartesian(accels, pos)
        return accels
